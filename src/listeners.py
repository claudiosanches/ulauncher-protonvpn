...
class KeywordQueryEventListener(EventListener):
    ...
    def _home_screen(self, pvpn):
        ...
        items.append(_item(
            "Refresh server list",
            "Download the country list from ProtonVPN" if not pvpn.has_cache()
    ...
    def _city_screen(self, pvpn, country_code, country_name, max_entries):
        ...
        if not pvpn.has_cities(country_code):
            ...
            items.append(_item(
                ...
                icon,
                _action_keep({"action": ACT_SHOW_CITIES, "code": country_code, "name": country_name}),
            ))
            return items
        ...
        if cities:
            for city in cities[:max_entries - 1]:
                ...
                desc = f"Features: {features}" if features else f"Connect to {name}, {country_code}"
                items.append(_item(
                    name,
                    desc,
                    flag,
                    _action({"action": ACT_CONNECT_CITY, "code": country_code, "city": name}),
                ))
        else:
            items.append(_item(
                "No cities available",
    ...
    def on_event(self, event, extension):
        ...
        if action == ACT_CONNECT_FASTEST:
            def _do_connect():
                ...
                Utils.notify(
                    "ProtonVPN",
                    (f"Connected. IP: {ip}" if ip else "Connected.") if success else "Connection failed.",
                )
        ...
        if action == ACT_CONNECT_COUNTRY:
            ...
            def _do_connect():
                ...
                Utils.notify(
                    "ProtonVPN",
                    (f"Connected to {code}. IP: {ip}" if ip else f"Connected to {code}.") if success else f"Connection to {code} failed.",
                )
            threading.Thread(target=_do_connect, daemon=True).start()
        ...
        if action == ACT_CONNECT_CITY:
            ...
            def _do_connect():
                ...
                Utils.notify(
                    "ProtonVPN",
                    (f"Connected to {city}, {code}. IP: {ip}" if ip else f"Connected to {city}, {code}.") if success else f"Connection to {city}, {code} failed.",
                )
            threading.Thread(target=_do_connect, daemon=True).start()
        ...
        if action == ACT_DISCONNECT:
            def _do_disconnect():
                ...
                Utils.notify("ProtonVPN", "Disconnected." if success else "Disconnect failed or already disconnected.")
        ...
        if action == ACT_REFRESH:
            def _do_refresh():
                ...
                if success:
                    Utils.notify("ProtonVPN", f"Server list saved. {len(pvpn.get_countries())} countries available.")
                else:
                    Utils.notify("ProtonVPN", "Refresh failed. Check that protonvpn CLI is signed in.")
